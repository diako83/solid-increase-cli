...
import { kbdata } from "@/mockdata/kettlebelldata";
import { useGlobalContext } from "@/app/context/store";
import { useRouter } from "next/navigation";
import { postKbWorkout } from "@/app/api/hello/route";
import DisplayKb from "@/app/components/displayWorkoutComp/displayKb";
const { v4: uuidv4 } = require("uuid");

function generateUUID() {
  return uuidv4();
}
const WorkoutKettlebells = () => {
  ...
  const [exercise, setExercise] = useState<IKbMovement>({
    id: uuidv4(),
    name: "",
    sets: 0,
    reps: 0,
  ...
  const [set, setSet] = useState<IKbMovementSet>({
    id: "",
    movements: [],
  });

  const [workout, setWorkout] = useState<IKbWorkout>({
    id: "",
    userId: "",
    title: "",
    description: "",
  ...
  const postData = async (
    ...
    try {
      const response: IResponseKb = await postKbWorkout(
        `http://localhost:9090/api/v1/kb`,
        data.token ?? "",
        workout
      );

      if (response.status == 200) {
        router.push("/activities/library/kb");
      }
  ...
  return (
    <div className="  bg-turquoise/[.90] rounded-3xl">
      {display ? (
        <DisplayKb
          workout={workout}
          postData={postData}
        ...
        <div className="  p-3 shadow-2xl  ">
          ...
          >
            ...
            <form
              ...
              method="POST"
            >
              <div className="lg:flex  text-turquoise/[.80] lg:justify-evenly">
                ...
                <div className="mb-2 mr-2 ml-2 text-center">
                  ...
                  <p className=" text-turquoise[0.90] text-lg">
                    {" "}
                    {exercise.name}
                  </p>
                 ...
                 "
                  >
                    ...
                    {kbdata.map((category: movement_categorys, index: number) =>
                      showOptions ? (
                        <optgroup
              ...
              <div className="mt-10 relative  flex flex-wrap justify-around">
                ...
                <div className="grid grid-col">
                  <label className="text-turquoise">Distance</label>
                  <select
            ...
            >
              ...
              <div
                id="scrollableDiv"
                className="relative overflow-x-auto shadow-md sm:rounded-2xl text-turquoise"
              >
                <table className="w-full text-sm text-left ">
                  ...
                  {set.movements.map((item: IKbMovement, index: number) => (
                    <TableHiitComp
                      key={index}
                      id={item.id}
                      name={item.name}
            ...
            >
              ...
              {workout.movementSets.map(
                (movementSet: IKbMovementSet, setIndex: number) => (
                  <div key={setIndex}>
                      ...
                      <div className=" overflow-x-auto shadow-md sm:rounded-2xl text-turquoise">
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                          ...
                          {movementSet.movements.map(
                            (movement: IKbMovement, movementIndex: number) => (
                              <SectionHiitComp
                                key={movementIndex}
                                id={movement.id}
                                name={movement.name}
...
export default WorkoutKettlebells;

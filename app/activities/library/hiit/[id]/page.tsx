...
const Page = () => {
  ...
  const [response, setResponse] = useState<IResponseHiit>();

  const id = pathname.substring(pathname.lastIndexOf("hiit/") + 5);

  useEffect(() => {
    const get = async () => {
      console.log("get kallad");
      if (user?.id != undefined) {
        const response: IResponseHiit = await fetchSingle(
          `http://localhost:9090/api/v1/hiit/single/${id}`,
          user.token ?? ""
        );
        ...
        >
          ...
          >
                        ...
                        border-solid border-2
                         focus:outline-none basis-1/4 mb-2 mr-2"
                            >
                              ...
                              <p className="text-sm">
                                Sets: {m.sets}, Reps:{m.sets}, Weight:{" "}
                                {m.weight}
                              </p>
                              <p className="text-sm">
            ...
            <button
              onClick={(e) =>
                deleteSingle(
                  `http://localhost:9090/api/v1/hiit/single/delete/${response?.data.id}`,
                  user.token

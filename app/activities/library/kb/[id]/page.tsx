...
const Page = () => {
  ...
  const id = pathname.substring(pathname.lastIndexOf("kb/") + 3);
    ...
    const get = async () => {
      ...
      if (user?.id != undefined) {
        const response: IResponseKb = await fetchSingle(
          `http://localhost:9090/api/v1/kb/single/${id}`,
          user.token ?? ""
        );
          ...
          >
            ...
            {response?.data.movementSets
              ? response?.data.movementSets.map(
                  (item: IKbMovementSet, index: number) => {
                    ...
                    return (
                      <div className="my-4" key={index}>
                        ...
                        {item.movements.map((m: IKbMovement, ind: number) => {
                          return (
                            <div
                              key={ind}
            ...
            <button
              onClick={(e) =>
                deleteSingle(
                  `http://localhost:9090/api/v1/kb/single/delete/${response?.data.id}`,
                  user.token
                ).then(() => router.push("/activities/library/kb"))
              }
